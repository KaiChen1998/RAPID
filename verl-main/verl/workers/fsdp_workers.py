...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        from transformers import AutoModelForCausalLM, AutoConfig, AutoModelForVision2Seq, AutoModelForImageTextToText
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def preprocess(self, messages_batch):
        ...
        for messages in messages_batch:
            ...
            if len(raw_prompt_ids) > 8192:
                raw_prompt_ids = raw_prompt_ids[:8192]
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def preprocess_multimodal(self, messages_batch):
        data = []
        for messages in messages_batch:
            row_dict = {}
...
class ReasonRolloutModelWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        hf_model_config = AutoConfig.from_pretrained(local_path, trust_remote_code=True)
        ...
        # from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode
        # from verl.workers.sharding_manager import VLLMOffloadShardingManager
        # log_gpu_memory_usage(f'Before building vllm reason rollout', logger=None)
        
        # if vllm_mode == 'spmd':
        #     rollout = vLLMRollout(model_path=local_path,
        #                             config=self.config.rollout,
        #                             tokenizer=self.tokenizer,
        #                             model_hf_config=hf_model_config,
        #                             device_mesh=rollout_device_mesh,
        #                             trust_remote_code=True)
        # else:
        #     raise NotImplementedError
        # log_gpu_memory_usage(f'After building vllm reason rollout', logger=None)
        # rollout_sharding_manager = VLLMOffloadShardingManager(inference_engine=rollout.inference_engine,
    ...
    #         log_gpu_memory_usage('After entering reason rollout sharding manager', logger=logger)

    #         prompts = self.rollout_sharding_manager.preprocess_data(prompts)
    #         output = self.rollout.generate_sequences(prompts=prompts)
    #         log_gpu_memory_usage('After reason rollout generation', logger=logger)
