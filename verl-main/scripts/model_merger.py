...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    elif "InternVLChatModel" in config.architectures[0]:
        # import sys
        # import importlib

        # # Add the parent directory to sys.path
        # sys.path.append('/cache/exps/verl_checkpoints/')

        # # Dynamically import the module using importlib
        # module_name = 'InternVL3-8B.modeling_internvl_chat'

        # # Import the module dynamically using the full name with a hyphen in the path
        # internvl_module = importlib.import_module(module_name)
        # auto_model = internvl_module.InternVLChatModel
    ...
    if "InternVLChatModel" in config.architectures[0]:
        with torch.device('cpu'):
            model = auto_model.from_pretrained(args.hf_model_path, config=config, torch_dtype=torch.bfloat16, trust_remote_code=True)
